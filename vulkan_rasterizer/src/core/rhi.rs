...
impl RHIDevice {
    ...
    pub fn new(window: &Arc<Window>, event_loop: &EventLoop<()>) -> Arc<RHIDevice> {
        ...
        let (physical_device, queue_family_index) =
            RHIDevice::select_physical_device(&instance, &surface, &device_extensions);

        let version = physical_device.api_version();
        info!(
            "Found physical device supported Vulkan version {:?}",
            version
        );
        ...
        info!(
            "Created device - Vulkan {:?} with extensions {:?}",
            device.api_version(),
...
impl Renderer {
    pub fn new(device: Arc<RHIDevice>, window: &Arc<Window>) -> Self {
        ...
        let memory_allocator =
            Arc::new(StandardMemoryAllocator::new_default(device.device.clone()));

        // Base triangle
        let vertex1 = VertexPositionOnly {
            position: [-0.5, -0.5],
        };
        let vertex2 = VertexPositionOnly {
            position: [0.0, 0.5],
        };
        let vertex3 = VertexPositionOnly {
            position: [0.5, -0.25],
        };
        ...
        let pipeline = Renderer::get_graphics_pipeline(
            device.device.clone(),
            vs,
            fs,
            render_pass.clone(),
    ...
    pub fn render_base(&mut self, window: &Arc<Window>) {
        ...
        // Renderpass
        let render_pass =
            Renderer::get_render_pass(self.device.device.clone(), &self.device.swapchain);

        // Framebuffer
        let framebuffer =
            Renderer::get_framebuffers(&self.device.swapchain_images, render_pass.clone());
